...
import { User, Course, Chunk, Quiz, QuizAttempt, Message, GroundingReference, ChunkWithEmbedding } from "./src/types.js";
...
const JWT_SECRET = process.env.JWT_SECRET || "course-pdf-agent-secret-key-998811";
...
async function startServer() {
  ...
  app.post("/api/auth/register", async (req: Request, res: Response) => {
    try {
      ...
      const exists = dbData.users.find((u) => u.email.toLowerCase() === email.toLowerCase());
      if (exists) {
  ...
  app.post("/api/auth/login", async (req: Request, res: Response) => {
    try {
      ...
      const dbData = db.read();
      const userRecord = dbData.users.find((u) => u.email.toLowerCase() === email.toLowerCase());
      if (!userRecord) {
  ...
  app.get("/api/dashboard/stats", authenticateToken, async (req: Request & { user?: any }, res: Response) => {
    try {
      ...
      const userChunks = dbData.chunks.filter((ch) => courseIds.includes(ch.courseId));
      const totalChunks = userChunks.length;

      const userQuizzes = dbData.quizzes.filter((q) => courseIds.includes(q.courseId));
      const totalQuizzes = userQuizzes.length;

      const userAttempts = dbData.attempts.filter((att) => courseIds.includes(att.courseId));
      const totalAttempts = userAttempts.length;

      let avgQuizScore = 0;
      if (totalAttempts > 0) {
        const sum = userAttempts.reduce((acc, att) => acc + (att.score / att.total) * 100, 0);
  ...
  app.get("/api/courses", authenticateToken, async (req: Request & { user?: any }, res: Response) => {
    try {
      const userId = req.user.id;
      const dbData = db.read();
      const userCourses = dbData.courses.filter((c) => c.userId === userId);
...
app.post("/api/courses/upload", authenticateToken, upload.single("file"), async (req: Request & { user?: any }, res: Response) => {
  try {
    const userId = req.user.id;
    if (!req.file) {
  ...
  try {
    ...
    if (extractedText.length < 50) {
      console.log(`WARNING: Very little text found! PDF might be scanned images.`);
    }
    
    // ===== SMART CHUNKING (NEW) =====
    console.log(`[PDF_PROCESSING] Applying smart chunking with semantic boundaries...`);
    ...
    console.log(`[PDF_PROCESSING] Created ${enrichedChunks.length} smart chunks`);

    // Generate embeddings in batch
    console.log(`[PDF_PROCESSING] Generating embeddings for ${enrichedChunks.length} chunks...`);
    ...
    if (embeddingService && enrichedChunks.length > 0) {
      try {
        ...
        console.log(`[PDF_PROCESSING] ✅ Generated ${embeddings.length} embeddings`);

        // Upsert to Pinecone
        if (vectorDb.isAvailable()) {
          console.log(`[PDF_PROCESSING] Upserting embeddings to Pinecone...`);
          ...
          console.log(`[PDF_PROCESSING] ✅ Embeddings upserted to Pinecone`);
        }
      } catch (embeddingError: any) {
        console.warn(`[PDF_PROCESSING] ⚠️ Embedding generation failed: ${embeddingError.message}`);
        console.warn(`[PDF_PROCESSING] Proceeding without embeddings. Will fall back to basic RAG.`);
      }
    }
    ...
    const finalizedC = reloadedDb.courses.find((c) => c.id === courseId);
    if (finalizedC) {
    ...
    console.log(`✅ Course ready! ${chunksList.length} chunks saved.\n`);

  } catch (processError: any) {
    console.error(`❌ Error:`, processError.message);
    const crashDb = db.read();
    const courseCrashed = crashDb.courses.find((c) => c.id === courseId);
    if (courseCrashed) {
  ...
  app.delete("/api/courses/:id", authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      dbData.attempts = dbData.attempts.filter((att) => att.courseId !== courseId);
  ...
  app.post("/api/courses/:id/chat", authenticateToken, async (req: Request, res: Response) => {
  try {
    const courseId = req.params.id;
    ...
    const courseChunks = dbData.chunks.filter((c) => c.courseId === courseId) as ChunkWithEmbedding[];
    ...
    if (courseChunks.length > 0) {
      try {
        console.log(`[CHAT] Using ADVANCED RAG (BM25 + Semantic + Query Expansion + Reranking + Validation)`);
        
        // 1. QUERY EXPANSION: Generate expanded queries for better coverage
        const expandedQueryResult = queryExpansionService.expandQuery(message, 5);
        searchMetadata.expanded = true;
        const queriesToSearch = [message, ...expandedQueryResult.expanded.slice(0, 3)];
        console.log(`[CHAT] Query expansion: ${queriesToSearch.length} queries to search`);
        
        // 2. HYBRID SEARCH: Execute BM25 and semantic search on all queries
        let allResults: Array<{ chunk: ChunkWithEmbedding; bm25Score: number; semanticScore: number }> = [];
        ...
        for (const query of queriesToSearch) {
          ...
          try {
            semanticResults = await semanticSearch.search(query, courseChunks, 8);
          } catch (semErr) {
            console.warn(`[CHAT] Semantic search skipped: ${(semErr as any).message}`);
          }
          
          // Combine results by chunk ID
          const resultMap = new Map<string, { chunk: ChunkWithEmbedding; bm25Score: number; semanticScore: number }>();
        ...
        const deduplicatedMap = new Map<string, { chunk: ChunkWithEmbedding; bm25Score: number; semanticScore: number; fusedScore: number }>();
        
        for (const result of allResults) {
          const existing = deduplicatedMap.get(result.chunk.id);
          const fusedScore = (result.bm25Score * 0.4 + result.semanticScore * 0.6); // 40% BM25, 60% semantic
        ...
        const rerankedResults = await rerankerService.rerank(
          message,
          sortedResults.map(r => ({ chunk: r.chunk, score: r.fusedScore }))
        );
        ...
        console.warn(`[CHAT] Advanced RAG failed, falling back to basic search: ${searchError.message}`);
...
User Question: ${message}`;
    ...
    const injectPageCitations = (response: string, context: string): string => {
      // Extract all page numbers from context [Chunk X, Page Y]
      const pageMatches = context.match(/Page (\d+)/g) || [];
      console.log(`[CITATION] Found ${pageMatches.length} page references in context:`, pageMatches);
      ...
      if (pages.size === 0) {
        console.log(`[CITATION] ⚠️ No pages found, returning response without citations`);
        return response;
      }
      ...
      let processed = response.replace(/\(\s*J['']utilise.*?\)$/gi, '').trim();
      
      console.log(`[CITATION] After cleanup:`, processed.substring(0, 100) + "...");
      ...
      const citedSentences = sentences.map((sentence, idx) => {
        ...
        if (lastPunct) {
          ...
          console.log(`[CITATION] Sentence ${idx} cited: ${result.substring(0, 80)}...`);
          return result;
        }
    ...
    const citedResponse = injectPageCitations(aiResponse, contextWindow.context);
    ...
    const sourcedChunks = hybridResults.length > 0 
      ? hybridResults.map(r => r.chunk) 
      : courseChunks.slice(0, 10);
    
    const validationResult = responseValidationService.validateResponse(citedResponse, sourcedChunks);
    searchMetadata.validationConfidence = validationResult.confidence;
    
    // Format response with validation disclaimer if needed
    const formattedResponse = responseValidationService.formatValidationForDisplay(citedResponse, validationResult);
    
    console.log(`[VALIDATION] Confidence: ${(validationResult.confidence * 100).toFixed(1)}%, Valid: ${validationResult.isValid}`);

    // Format references from search results
    const references = hybridResults.slice(0, contextWindow.usedChunks).map((r, i) => ({
    ...
    console.log(`✅ Response generated with validation (${contextWindow.usedChunks} refs, ${contextWindow.totalTokens} tokens, confidence: ${(validationResult.confidence * 100).toFixed(1)}%)\n`);
  ...
  app.post("/api/courses/:id/summary", authenticateToken, async (req: Request, res: Response) => {
    try {
      const courseId = req.params.id;
      const dbData = db.read();
      const course = dbData.courses.find((c) => c.id === courseId);
      ...
      const courseChunks = dbData.chunks.filter((ch) => ch.courseId === courseId);
      if (courseChunks.length === 0) {
        return res.status(400).json({ error: "Course chunks not found. Please wait for processing to finish." });
      }
      ...
      let generatedSummary = {
        overview: "A concise overview summarizing the uploaded educational textbook material. Configure your GROQ_API_KEY to trigger the true Summary Agent output.",
        chapters: [
          {
            title: "Chapter 1: Dynamic Introduction",
            summary: "An introduction compiled from initial concepts extracted from course materials.",
            takeaways: ["Key takeaway reference 1", "Core concepts are introduced in sequential forms."],
          },
        ],
        keyConcepts: [
          {
            term: "Course Outline",
            definition: "The core architecture or instructional handbook covering learning objectives.",
          },
        ],
      };
...
Return ONLY valid JSON, no other text.`;
        ...
        try {
          const completion = await groq.chat.completions.create({
            ...
            messages: [
              {
                role: "system",
                content: "You are a JSON generator. Return only valid JSON matching the requested schema, no additional text.",
              },
              {
                role: "user",
          ...
          return res.status(500).json({ error: `Summary generation crashed: ${sum_err.message || sum_err}` });
        }
      }
  ...
  app.post("/api/courses/:id/quiz", authenticateToken, async (req: Request, res: Response) => {
  try {
    const courseId = req.params.id;
    const dbData = db.read();
    const course = dbData.courses.find((c) => c.id === courseId);
    ...
    if (courseChunks.length === 0) {
      return res.status(400).json({ error: "No course content found. Please wait for PDF processing to complete." });
    }

    // Combine all chunks to get full text
    const fullText = courseChunks.map(c => c.text).join(" ").substring(0, 3000);
...
Make sure the questions are based on the text above.`;
      ...
      const completion = await groq.chat.completions.create({
        ...
        messages: [
          { 
            role: "system", 
            content: "You are a quiz generator. Return ONLY valid JSON. No other text." 
          },
          { 
            role: "user", 
            content: quizPrompt 
  ...
  const fallbackQuestions = [
    {
      ...
      options: ["Based on the course materials", "General knowledge", "Not specified", "All of the above"],
      correctAnswerIndex: 0,
      explanation: "The course materials contain information about this topic."
    },
    {
      question: "What does the course PDF contain?",
      options: ["Only images", "Text content that was extracted", "Empty pages", "Video links"],
      correctAnswerIndex: 1,
      explanation: "The PDF was successfully processed and text was extracted."
    },
    {
      question: "How many chunks were extracted from your PDF?",
      options: [`${courseChunks.length} chunks`, "No chunks", "100 chunks", "Unknown"],
      correctAnswerIndex: 0,
...
await db.write(reloadDb);
  ...
  app.post("/api/quizzes/:id/submit", authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    if (!quiz) {
      const courseWithQuiz = dbData.courses.find((c) => c.quiz && c.quiz.id === quizId);
      if (courseWithQuiz) {
    ...
    quiz.questions.forEach((q) => {
      ...
      if (studentSelect !== undefined && studentSelect === q.correctAnswerIndex) {
        score++;
      }
    });
...
app.get("/api/test/:courseId", authenticateToken, async (req: Request & { user?: any }, res: Response) => {
  try {
    ...
    res.json({
      ...
      sampleText: chunks.length > 0 ? chunks[0].text.substring(0, 300) : "No text found"
    });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
  ...
  try {
    if (vectorDb.isAvailable()) {
      ...
      console.warn("[SERVER] ⚠️ Vector DB not available. Embeddings will be disabled.");
    }
  } catch (error) {
    console.warn("[SERVER] Vector DB initialization failed. System will continue without vector search.", error);
  }

  app.listen(PORT, "0.0.0.0", () => {
    console.log(`AI Course PDF Assistant server is booted on port http://localhost:${PORT}`);
  });
}
